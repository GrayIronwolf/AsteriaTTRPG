...
function renderPlayerExtras(id){
  ...
  if($('pStatsLarge')){
    if(typeof renderCharacteristicCP==='function') renderCharacteristicCP(id);
    else $('pStatsLarge').innerHTML=Object.entries(c.characteristics||{}).map(([k,v])=>`<div class="stat-card"><small>${tierOf(v)}</small><span>${statLabels[k]||k}</span><br><b>${v}</b><p class="muted">${k[0].toUpperCase()+k.slice(1)}</p></div>`).join('');
  }
...
function ensureCharacterDashboardLink(id){
  if(!chars?.[id]) return null;
  const stamp=new Date().toISOString();
  const existing=chars[id].dashboard||{};
  const dashboard=Object.assign({
    id:'dashboard-'+id,
    characterId:id,
    route:'player',
    title:(chars[id].name||'Character')+' Dashboard',
    created:existing.created||stamp,
    updated:stamp
  },existing,{characterId:id,route:'player'});
  chars[id].dashboard=dashboard;
  window.AsteriaCharacterDashboards=window.AsteriaCharacterDashboards||{};
  window.AsteriaCharacterDashboards[id]=dashboard;
  return dashboard;
}
function selectAccountCharacterCard(id){
  ...
  toast((chars[id].name||'Character')+' selected. Double-click the card to open the dashboard.');
}
window.ensureCharacterDashboardLink=ensureCharacterDashboardLink;
window.selectAccountCharacterCard=selectAccountCharacterCard;
const asteria13OldCurrentPlayerId=currentPlayerId;
currentPlayerId=function(){
  if(['player','account'].includes(session.role)) return session.character || (accountUsers[session.account||session.uid||session.user||session.email||'local-player']?.characters?.[0]) || selected;
  return selected;
}
function renderPlayerHome(){
  loadAccountState();
  const account=session.account||session.uid||session.user||session.email||'local-player';
...
let activeTalentTier=1;
let selectedTalentNode=null;
function talentClassFromText(text=''){
  const k=String(text || '').toLowerCase();
  const out=[];
  const trees=typeof asteriaClassTalentTrees!=='undefined'?asteriaClassTalentTrees:{};
  Object.entries(trees).forEach(([key,tree])=>{
    const label=String(tree?.label || key);
    const variants=[key,label,label.replace(/\s+/g,''),key.replace(/-/g,'')].map(value=>String(value).toLowerCase());
    if(variants.some(value=>value&&k.includes(value)))out.push(key);
  });
  if(k.includes('blood hunter')||k.includes('bloodhunter'))out.push('bloodhunter');
  return [...new Set(out)];
}
...
function setTalentTier(tier){
  activeTalentTier=Math.max(1,Math.min(5,Number(tier)||1));
...
function talentRankRows(id,name,max,desc){
  const c=chars[id]||{};
  const current=c.talents?.[name]?.rank||0;
  const staged=talentDrafts[id]?.[name]||0;
  const detail=typeof getBloodhunterDetail==='function'?getBloodhunterDetail(name):null;
  const rows=[];
  for(let rank=1;rank<=max;rank++){
    const unlocked=rank<=current;
    const pending=rank>current&&rank<=current+staged;
    const next=rank===current+staged+1;
    const text=(detail?.ranks?.[rank-1]||`${desc} Rank ${rank}.`).replace(/"/g,'&quot;');
    rows.push(`<li class="${unlocked?'unlocked':''} ${pending?'pending':''} ${next?'next':''}"><span>Rank ${rank}</span><p>${escapeHtml(text)}</p></li>`);
  }
  return rows.join('');
}
function renderTierTalentCard(id,talent,locked,tier){
  const c=chars[id]||{};
  const [name,type,max,desc]=talent;
  const current=c.talents?.[name]?.rank||0;
  const staged=talentDrafts[id]?.[name]||0;
  const shown=current+staged;
  ...
  return `<article class="tier-talent-card ${locked?'locked':''} ${current>0?'unlocked':''} ${staged>0?'pending':''}">
    ...
    <p>${escapeHtml(desc)}</p>
    <ol class="talent-rank-ladder">${talentRankRows(id,name,max,desc)}</ol>
    <div class="talent-purchase-row">
      <button ${locked||!staged?'disabled':''} onclick="stageTalent('${safeName}',${max},-1)">-</button>
...
renderTalentTreeUI=function(id=currentPlayerId()){
  ...
  if(!c.talentClass||!asteriaClassTalentTrees[c.talentClass])c.talentClass=c.talentClasses[0]||'ranger';
  if(activeTalentFocus==='multi'||!asteriaClassTalentTrees[activeTalentFocus])activeTalentFocus=c.talentClass;
  const classKey=activeTalentFocus||c.talentClass;
  const tree=asteriaClassTalentTrees[classKey]||asteriaClassTalentTrees.ranger;
  c.talentClass=classKey;
  const cost=stagedTalentCost(id);
  const remaining=Math.max(0,(c.tp||0)-cost);
  const tierIndex=Math.max(0,Math.min(4,activeTalentTier-1));
  const tier=(tree.tiers||[])[tierIndex]||null;
  const locked=!!tier&&(c.level||0)<Number(tier.unlock||1);
  const tierTabs=[1,2,3,4,5].map(number=>{
    const tabTier=(tree.tiers||[])[number-1];
    const cls=[number===activeTalentTier?'active':'',!tabTier?'empty':((c.level||0)<Number(tabTier.unlock||1)?'locked':'')].join(' ');
    return `<button class="${cls}" onclick="setTalentTier(${number})">Tier ${number}</button>`;
  }).join('');
  host.innerHTML=`<div class="section-head"><div><h3>Class Talent Tree</h3><p class="muted">Spend TP to unlock talents and buy new ranks. Pick a tier above, then stage ranks with + / - and apply the purchase.</p></div><div class="tp-box">Available TP <b id="tpAvailable">${remaining}</b><small>Staged cost: ${cost}</small></div></div>
    <div class="talent-toolbar tier-talent-toolbar"><label>Class Tree <select id="talentClassSelect" onchange="changeTalentClass()">${Object.entries(asteriaClassTalentTrees).map(([k,v])=>`<option value="${k}" ${k===classKey?'selected':''}>${v.label}</option>`).join('')}</select></label><button class="primary" onclick="applyTalentRanks()">Apply Talent Changes</button></div>
    <div class="tier-talent-shell">
      <div class="talent-tier-tabs">${tierTabs}</div>
      ${c.talentClasses.length>1?`<div class="multi-class-note">This character has multiple class trees available. TP is shared across all class talent purchases.</div>`:''}
      ${tier?`<section class="tier-talent-workspace ${locked?'locked-tier':''}">
        <div class="tier-workspace-head"><div><p class="eyebrow">${escapeHtml(tree.label)} Talent Tree</p><h3>Tier ${activeTalentTier}: ${escapeHtml(tier.name)}</h3><small>${locked?'Unlocks at Level '+tier.unlock:'Unlocked'}${tier.capstone?' / GM-approved capstone tier':''}</small></div><span>${(tier.talents||[]).length} talents</span></div>
        <div class="tier-talent-grid">${(tier.talents||[]).map(talent=>renderTierTalentCard(id,talent,locked,tier)).join('')}</div>
      </section>`:`<section class="tier-talent-empty"><h3>Tier ${activeTalentTier}</h3><p class="muted">No talents have been added for this tier yet.</p></section>`}
    </div>`;
};
...
renderTalentTreeUI=function(id=currentPlayerId()){asteria132OldRenderTalentTreeUI(id);const c=chars[id];const host=document.querySelector('#talents .tier-talent-shell')||document.querySelector('#talents .visual-tree-shell');if(host&&getCharacterTalentClasses(c).includes('bloodhunter')&&!host.querySelector('.bloodhunter-system-card'))host.insertAdjacentHTML('afterbegin',renderBloodhunterSystemPanel());};
...
function renderCoinPanel(){
  ...
  box.innerHTML=ASTERIA_V133_COINS.map(coin=>{
    ...
    return `<div class="coin-row"><div><b>${coin.label}</b><small>${coin.term}</small></div><div class="coin-controls"><input id="coin_${coin.key}" type="number" min="0" value=""><button onclick="adjustCoin('${coin.key}',-1)">−</button><button onclick="adjustCoin('${coin.key}',1)">+</button><span>Have: ${count}</span></div></div>`;
  }).join('');
...
function adjustCoin(key,sign){
  const c=chars[currentPlayerId()]; if(!c) return; ensureWebInventory(currentPlayerId());
  const amount=Number($('coin_'+key)?.value);
  if(!Number.isSafeInteger(amount)||amount<1)return toast('Enter a whole amount of at least 1.');
...
function combatDefenseForCharacter(c){return window.AsteriaArmour?.calculateCharacterAC?.(c||{})?.finalAC??Math.max(1,Number(c?.naturalAC||c?.raceInfo?.naturalAC||1));}
...
function renderDashboardClassInfo(id=currentPlayerId?.()){
  ...
  const host=document.querySelector('.character-profile-panel');
  if(!host)return;
  let panel=host.querySelector('#characterClassInfo');
  if(!panel){
    panel=document.createElement('div');
    panel.id='characterClassInfo';
    panel.className='class-info-sync-panel';
    host.appendChild(panel);
  }
  ...
  const rows=[
    ...
    ['Secondary Stat', info.secondaryStat||info.secondary_stat],
    ['Combat Style', info.combatStyle||info.combat_style],
    ['Magic Type', info.magicType||info.magic_type],
    ['Difficulty', info.difficulty]
  ].filter(([,value])=>String(value||'').trim());
  panel.innerHTML=`<h4>Class Information</h4>${rows.length?rows.map(([label,value])=>`<div class="class-info-row"><span>${label}</span><b>${value}</b></div>`).join(''):'<p class="muted smallnote">Class information will sync from Character Forge.</p>'}`;
}
...
(function(){
  ...
  function compendiumItem(itemId){
    ...
    const record=Object.assign({id:itemId||uid('item'),name:item.name||itemId||'Unknown Item',type:item.type||'Item',qty:item.qty||1,desc:item.desc||item.description||'Compendium-linked item snapshot.',weight:item.weight||0,tags:item.tags||[],version:item.version||'compendium-current',sourcePage:item.sourcePage||`content/Items/${item.name||itemId||'Unknown Item'}.md`},item);
    return window.AsteriaMarketPricing?.normalizeMarketPricing(record,{legacy:true,removeLegacy:true})||Object.assign(record,{marketValue:0,marketPrice:0});
  }
  function snapshotItem(itemRef,source='Transaction'){
    ...
    const snapshot=Object.assign({},base,{snapshotId:uid('itemSnap'),snapshotVersion:base.version||'compendium-current',sourcePage:base.sourcePage||'Compendium Item Page',source,receivedAt:nowStamp(),gmOnly:!!base.gmOnly,hidden:!!base.hidden,cursed:!!base.cursed,questItem:!!base.questItem});
    return window.AsteriaMarketPricing?.normalizeMarketPricing(snapshot,{legacy:true,removeLegacy:true})||Object.assign(snapshot,{marketValue:0,marketPrice:0});
  }
  ...
  function currencyInputs(){return currencyTypes.map(k=>`<label>${currencyLabels[k]}<input id="txCur_${k}" type="number" min="0" value=""></label>`).join('');}
...
(function(){
  ...
  function craftingArmourPieceOptions(selected=''){
    const pieces=Object.values(window.AsteriaArmour?.ARMOUR_PIECES||{});
    return pieces.map(piece=>`<option value="${piece.id}" ${piece.id===selected?'selected':''}>${piece.name} (${Math.round(piece.percentile*100)}%)</option>`).join('')||'<option value="cuirass">Cuirass (100%)</option><option value="heaterShield">Heater Shield (55%)</option>';
  }
  function craftingArmourTypeOptions(selected='Medium Armour'){
    const types=Object.values(window.AsteriaArmour?.ARMOUR_TYPES||{});
    return types.map(type=>`<option value="${type.name}" ${type.name===selected?'selected':''}>${type.name}</option>`).join('')||'<option>Cloth</option><option>Leather</option><option>Light Armour</option><option selected>Medium Armour</option><option>Heavy Armour</option>';
  }
  function calculateCraftedArmour(p,template,material,q){
    if(!/armou?r|shield/i.test(`${p.template||''} ${template.type||''}`))return null;
    const engine=window.AsteriaArmour;
    const pieceId=p.armourPiece||(p.template==='Shield'?'heaterShield':'cuirass');
    const piece=engine?.ARMOUR_PIECES?.[pieceId];
    const materialRecord=engine?.resolveMaterialBaseAC?.({material:p.material});
    const materialBaseAC=materialRecord?.configured?Number(materialRecord.value):Number(material.ac||0);
    const quality=engine?.resolveQuality?.({quality:p.quality})||{modifier:Number(q.ac||0),configured:true,name:p.quality};
    const crafting=engine?.craftArmourBaseAC?.(materialBaseAC,quality)||{materialBaseAC,quality,craftedBaseAC:materialBaseAC+Number(quality.modifier||0),destroyed:materialBaseAC+Number(quality.modifier||0)<0,survives:materialBaseAC+Number(quality.modifier||0)>=0};
    const item={name:p.itemName||p.projectName||`${p.material} ${piece?.name||p.template}`,type:p.template,material:p.material,materialBaseAC,quality:p.quality,armourPieceType:pieceId,armourType:p.armourType||'Medium Armour'};
    const pieceResult=engine?.calculateArmourPieceAC?.(item)||{item,piece,location:piece?.location||'',materialBaseAC,quality,modifiedBaseAC:crafting.craftedBaseAC,percentile:Number(piece?.percentile||0),contribution:crafting.craftedBaseAC*Number(piece?.percentile||0),valid:Boolean(piece),errors:piece?[]:['Missing armour-piece type.']};
    return {item,piece:pieceResult,crafting,materialSource:materialRecord?.source||'legacy-material-fallback'};
  }
  function calcPreview(p){
    ...
    const armour=calculateCraftedArmour(p,template,material,q);
    const ac=armour?Number(armour.piece.contribution||0):0;
    ...
    return {template,material,cls,q,ac,dmgMod,tier,out,xp,value,cap:affinityCap(p.affinity),armour,destroyed:Boolean(armour?.crafting?.destroyed)};
  }
  function finalItemSnapshot(p){const calc=calcPreview(p); const isPotion=p.template==='Potion'; const isSoul=p.template==='Soul Stone Focus'||p.material==='Soulstone'; const item={
    ...
    baseDamage:calc.template.baseDamage, damageModifier:calc.dmgMod, marketValue:Number(p.marketValue||0), marketPrice:Number(p.marketPrice||0),
    materialBaseAC:calc.armour?.piece?.materialBaseAC, craftedBaseAC:calc.armour?.crafting?.craftedBaseAC, armourContribution:calc.armour?.piece?.contribution,
    armourPieceType:calc.armour?.piece?.piece?.id, armourType:p.armourType||'', allowedSlots:calc.armour?.piece?.location?[calc.armour.piece.location]:[], destroyedDuringCrafting:calc.destroyed,
    ...
    soulStone:isSoul?{rank:p.soulRank,soulPoints:Number(p.soulPoints||0),craftMaterial:true,enchantMaterial:true}:null,
  ...
  }; return window.AsteriaMarketPricing?.createAsteriaItem(item)||item;}
  function serializeProjectForm(prefix='craft'){const g=id=>document.getElementById(prefix+id); return {
    ...
    template:g('Template')?.value||'Sword', armourPiece:g('ArmourPiece')?.value||'', armourType:g('ArmourType')?.value||'Medium Armour', material:g('Material')?.value||'Iron', itemClass:g('Class')?.value||'Common', quality:g('Quality')?.value||'Average', craftTier:g('Tier')?.value||'Simple',
  ...
  window.completeCraftProject=function(id){const s=state(); const p=s.projects.find(x=>x.id===id); if(!p)return; p.finalRoll=(Number(p.roll)||0)+(Number(p.skillMod)||0)+(Number(p.toolMod)||0)+(Number(p.materialMod)||0)+(Number(p.environmentMod)||0)+(Number(p.gmMod)||0); p.outcome=p.outcome||outcomeFromTotal(p.finalRoll); const calc=calcPreview(p); try{p.finalSnapshot=finalItemSnapshot(p);}catch(error){notifyCraft('medium','Invalid Market Information',error.message||'Market Value must be lower than Market Price.','warning');return;} p.completedAt=now(); p.status=calc.destroyed?'Destroyed':'Completed'; write(s);
    const notificationLevel=(calc.destroyed||p.itemClass==='Relic'||p.itemClass==='Legendary'||p.craftTier==='Savant'||p.quality==='Masterwork'||p.outcome==='Catastrophic Failure')?'major':'medium';
    const notificationTitle=calc.destroyed?'Armour Destroyed During Crafting':p.outcome==='Catastrophic Failure'?'Catastrophic Crafting Failure':'Item Crafted';
    const notificationMessage=calc.destroyed?`${p.projectName}: Material Base AC plus quality fell below zero. The armour piece was destroyed.`:`${p.projectName}: ${p.outcome}. XP ${calc.xp}.`;
    notifyCraft(notificationLevel,notificationTitle,notificationMessage,calc.destroyed||p.outcome==='Catastrophic Failure'?'warning':'crafting',{important:notificationLevel==='major'});
    logSession('Crafting',`Craft ${calc.destroyed?'destroyed':'completed'}: ${p.projectName} — ${p.outcome}`,{project:p.projectName,character:chars?.[p.characterId]?.name,skill:p.skill,rank:p.skillRank,affinityCap:calc.cap,materials:p.materials,tools:p.tools,roll:p.roll,finalRoll:p.finalRoll,outcome:p.outcome,xp:calc.xp,item:p.finalSnapshot.name,stats:`Piece AC ${calc.ac}, Damage modifier ${calc.dmgMod}`,destroyed:calc.destroyed,destination:p.destination},'Public');
    if(!calc.destroyed&&window.asteriaTransaction?.create){ window.asteriaTransaction.create({type:'craftedItem',source:'Crafting Project',playerId:p.characterId,items:[p.finalSnapshot],destination:p.destination==='partyLoot'?'partyLoot':'inventory',summary:`Crafted ${p.finalSnapshot.name} (${p.outcome})`,gmApproved:true,currency:{},materialsConsumed:p.materials}); }
    else if(!calc.destroyed){ const c=chars?.[p.characterId]; if(c){c.inventory=c.inventory||[]; c.inventory.push(p.finalSnapshot); if(typeof saveAsteriaState==='function')saveAsteriaState();} }
    if(calc.xp>0 && typeof adjustCharacterResource==='function'){} // Crafting skill XP hook reserved for Skill System integration.
    addCraftLog({type:calc.destroyed?'Armour Destroyed':'Project Completed',project:p.projectName,character:chars?.[p.characterId]?.name,skill:p.skill,skillRank:p.skillRank,affinityCap:calc.cap,materials:p.materials,tools:p.tools,roll:p.roll,finalRoll:p.finalRoll,outcome:p.outcome,xp:calc.xp,item:p.finalSnapshot.name,stats:`Piece AC ${calc.ac}, Damage +${calc.dmgMod}`,destroyed:calc.destroyed,destination:p.destination});
  };
  window.updateCraftPreview=function(){const p=serializeProjectForm('craft'); p.finalRoll=(p.roll||0)+p.skillMod+p.toolMod+p.materialMod+p.environmentMod+p.gmMod; p.outcome=p.outcome||outcomeFromTotal(p.finalRoll); const calc=calcPreview(p); const validation=window.AsteriaMarketPricing?.validateMarketPricing(p); const el=document.getElementById('craftPreview'); if(!el)return; const piece=calc.armour?.piece; const crafting=calc.armour?.crafting; el.innerHTML=`<div class="craft-preview-grid"><div><small>Final Item</small><b>${p.itemName||p.projectName||p.template}</b><span>${p.itemClass} ${p.quality} ${p.material} ${p.template}</span></div><div><small>${piece?'Armour Piece AC':'AC'}</small><b>${calc.destroyed?'Destroyed':`${calc.ac>=0?'+':''}${Number(calc.ac).toFixed(2).replace(/\.00$/,'')}`}</b><span>${piece?`${crafting.materialBaseAC} ${crafting.quality.modifier>=0?'+':''}${crafting.quality.modifier} x ${Math.round(piece.percentile*100)}%`:'Not an armour item'}</span></div><div><small>Damage Modifier</small><b>${calc.dmgMod>=0?'+':''}${calc.dmgMod}</b><span>Base ${calc.template.baseDamage}</span></div><div><small>Market Value</small><b>${p.marketValue} Marks</b><span>Player selling baseline</span></div><div><small>Market Price</small><b>${p.marketPrice} Marks</b><span>Player purchase baseline</span></div><div><small>Crafting XP</small><b>${calc.xp}</b><span>${calc.tier.xp} x outcome + ${p.skillRank}</span></div><div><small>Affinity Cap</small><b>${calc.cap}</b><span>Affinity ${p.affinity}</span></div><div><small>Outcome</small><b>${p.outcome}</b><span>Final roll ${p.finalRoll||0}</span></div></div>${validation&&!validation.valid?`<p class="craft-destruction-warning">${validation.errors[0]}</p>`:''}${calc.destroyed?'<p class="craft-destruction-warning">Crafted Base AC is below zero. This armour will be destroyed and cannot enter inventory.</p>':''}<p class="muted smallnote">${calc.tier.desc}</p>`;};
  ...
  function craftingFormHTML(prefix='craft'){
    return `<input type="hidden" id="${prefix}Id"><input type="hidden" id="${prefix}Status" value="Draft"><div class="craft-form-grid">
      ...
      <label>Item Template<select id="${prefix}Template" onchange="updateCraftPreview()">${opt(BASE_TEMPLATES)}</select></label><label>Armour Piece<select id="${prefix}ArmourPiece" onchange="updateCraftPreview()">${craftingArmourPieceOptions('cuirass')}</select></label><label>Armour Type<select id="${prefix}ArmourType" onchange="updateCraftPreview()">${craftingArmourTypeOptions('Medium Armour')}</select></label><label>Base Material<select id="${prefix}Material" onchange="updateCraftPreview()">${opt(BASE_MATERIALS,'Steel')}</select></label><label>Item Class / Rarity<select id="${prefix}Class" onchange="updateCraftPreview()">${opt(ITEM_CLASSES,'Common')}</select></label>
      ...
      <label>Required Skill<input id="${prefix}Skill" value="Smithing"></label><label>Skill Rank<select id="${prefix}SkillRank" onchange="updateCraftPreview()">${opt(SKILL_RANK_BONUS,'Initiate')}</select></label><label>Skill Affinity<input id="${prefix}Affinity" type="number" min="1" max="100" value="" oninput="updateCraftPreview()"></label>
    ...
    </div><details open><summary>Market Information</summary><div class="craft-form-grid"><label>Market Value (Marks)<input id="${prefix}MarketValue" type="number" min="0" step="0.01" value="" oninput="updateCraftPreview()"></label><label>Market Price (Marks)<input id="${prefix}MarketPrice" type="number" min="0" step="0.01" value="" oninput="updateCraftPreview()"></label></div><p class="muted smallnote">Market Value is the player selling baseline. Market Price is the player purchase baseline. Use 0 / 0 for items that are not normally tradeable.</p></details><details><summary>Potion Output</summary><div class="craft-form-grid"><label>Strength<select id="${prefix}PotionStrength">${optArr(POTION_STRENGTH,'Processed')}</select></label><label>Level<select id="${prefix}PotionLevel">${optArr(POTION_LEVEL,'Basic')}</select></label><label>Effect Value<input id="${prefix}PotionEffect" placeholder="2d6 healing, +10 MP..."></label><label>Duration<input id="${prefix}PotionDuration" placeholder="1 hour"></label><label>Side Effects<input id="${prefix}PotionSide" placeholder="Nausea, mana burn..."></label></div></details>
    <details><summary>Soul Stone Fields</summary><div class="craft-form-grid"><label>Rank<select id="${prefix}SoulRank">${optArr(SOUL_STONE_RANKS,'Basic')}</select></label><label>Soul Points<input id="${prefix}SoulPoints" type="number" value=""></label></div></details>
    <details open><summary>Manual d100 Crafting Check</summary><div class="craft-form-grid compact"><label>Base d100 Roll<input id="${prefix}Roll" type="number" min="1" max="100" value="" oninput="updateCraftPreview()"></label><label>Skill Mod<input id="${prefix}SkillMod" type="number" value="" oninput="updateCraftPreview()"></label><label>Tool Mod<input id="${prefix}ToolMod" type="number" value="" oninput="updateCraftPreview()"></label><label>Material Mod<input id="${prefix}MaterialMod" type="number" value="" oninput="updateCraftPreview()"></label><label>Environment Mod<input id="${prefix}EnvMod" type="number" value="" oninput="updateCraftPreview()"></label><label>GM Mod<input id="${prefix}GmMod" type="number" value="" oninput="updateCraftPreview()"></label><label>Outcome Override<select id="${prefix}Outcome" onchange="updateCraftPreview()"><option value="">Auto from roll</option><option>Success</option><option>Partial Success</option><option>Major Failure</option><option>Catastrophic Failure</option></select></label></div></details>
...
(function(){
  ...
  function serialize(prefix='ench'){const g=id=>document.getElementById(prefix+id); const p={id:g('Id')?.value||uid('ench'),status:g('Status')?.value||'Draft',name:g('Name')?.value||'Unnamed Enchantment',characterId:g('Character')?.value||selected,sourceSpell:g('Spell')?.value||'fireball',spellRank:g('Rank')?.value||'I',bindingType:g('Binding')?.value||'Weapon Binding',boundItem:g('Item')?.value||'',itemClass:g('Class')?.value||'Common',material:g('Material')?.value||'Steel',requiredSkill:g('Skill')?.value||'Enchanting',requiredRank:g('SkillRank')?.value||'Initiate',soulRank:g('SoulRank')?.value||'Basic',resourceReq:g('Resource')?.value||'',activation:g('Activation')?.value||'Action',trigger:g('Trigger')?.value||'On use',cooldown:g('Cooldown')?.value||'',duration:g('Duration')?.value||'',chargesMax:Number(g('ChargesMax')?.value||0),chargesCurrent:Number(g('ChargesCurrent')?.value||0),effect:g('Effect')?.value||'',gmNotes:g('GMNotes')?.value||'',visibility:g('Visibility')?.value||'Public',marketValue:Number(g('MarketValue')?.value||0),marketPrice:Number(g('MarketPrice')?.value||0),roll:Number(g('Roll')?.value||0),skillMod:Number(g('SkillMod')?.value||0),spellMod:Number(g('SpellMod')?.value||0),materialMod:Number(g('MaterialMod')?.value||0),soulMod:Number(g('SoulMod')?.value||0),toolMod:Number(g('ToolMod')?.value||0),envMod:Number(g('EnvMod')?.value||0),gmMod:Number(g('GMMod')?.value||0),outcome:g('Outcome')?.value||''}; p.finalRoll=p.roll+p.skillMod+p.spellMod+p.materialMod+p.soulMod+p.toolMod+p.envMod+p.gmMod; p.warnings=validateProject(p); const pricingValidation=window.AsteriaMarketPricing?.validateMarketPricing(p); if(pricingValidation&&!pricingValidation.valid)p.warnings.push(pricingValidation.errors[0]); p.risk=riskFromScore(instabilityScore(p)); p.stability=p.risk; p.updatedAt=now(); return p;}
  function outcome(total){return total>=75?'Success':total>=50?'Partial Success':total>=25?'Major Failure':'Catastrophic Failure';}
  function snapshot(p){const sp=spell(p.sourceSpell); const item={id:uid('enchitem'),name:p.boundItem||`${p.name} Item`,type:'Enchanted Item',rarity:p.itemClass,itemClass:p.itemClass,material:p.material,marketValue:Number(p.marketValue||0),marketPrice:Number(p.marketPrice||0),source:'Enchantment Project',enchanted:true,enchantment:{name:p.name,sourceSpellId:sp.id,sourceSpell:sp.name,spellRank:p.spellRank,rankLabel:RANKS[p.spellRank],bindingType:p.bindingType,chargesMax:p.chargesMax,chargesCurrent:p.chargesCurrent,cooldown:p.cooldown,trigger:p.trigger,activation:p.activation,duration:p.duration,stability:p.stability,instabilityRisk:p.risk,effectSummary:p.effect||sp.summary,visibility:p.visibility,gmHidden:p.visibility==='GM-only',soulStone:p.soulRank,sourceSpellVersion:'Spell Page v1',materialVersion:'Material System v1',createdAt:now(),snapshotLocked:true},desc:`${p.bindingType} linked to ${sp.name}. ${p.effect||sp.summary}`}; return window.AsteriaMarketPricing?.createAsteriaItem(item)||item;}
  ...
  function installEnchantMarketFields(prefix='ench'){
    if(document.getElementById(prefix+'MarketValue'))return true;
    const notes=document.getElementById(prefix+'GMNotes');
    const grid=notes?.closest('.enchant-form-grid');
    if(!grid)return false;
    grid.insertAdjacentHTML('afterend',`<details open class="enchant-market-information"><summary>Market Information</summary><div class="enchant-form-grid compact"><label>Market Value (player sale baseline)<input id="${prefix}MarketValue" type="number" min="0" step="0.01" value="" oninput="updateEnchantPreview()"></label><label>Market Price (player purchase baseline)<input id="${prefix}MarketPrice" type="number" min="0" step="0.01" value="" oninput="updateEnchantPreview()"></label></div><p class="smallnote">Use 0 / 0 for an item that is not normally tradeable. When both are positive, Market Value must be lower than Market Price.</p></details>`);
    return true;
  }
  const enchantPricingObserver=new MutationObserver(()=>{if(installEnchantMarketFields('ench'))enchantPricingObserver.disconnect();});
  if(!installEnchantMarketFields('ench'))enchantPricingObserver.observe(document.documentElement,{childList:true,subtree:true});
...
(function(){
      ...
      <div class="gm-encounter-builder-grid">
        <section class="gm-encounter-search-panel">
          ...
          <div class="gm-enemy-search-controls">
            ...
            <label>Number<input id="gmEncounterQty" type="number" min="1" max="20" value=""></label>
          ...
          <details class="gm-manual-enemy">
            ...
            <div class="gm-manual-enemy-grid">
              ...
              <label>HP<input id="gmManualEnemyHP" type="number" value=""></label>
              <label>AC<input id="gmManualEnemyAC" type="number" value=""></label>
              <label>Initiative<input id="gmManualEnemyInit" type="number" value=""></label>
  ...
  window.gmAddCreatureToEncounter=function(id){
    ...
    const qty=Number(document.getElementById('gmEncounterQty')?.value);
    if(!Number.isSafeInteger(qty)||qty<1||qty>20)return toast?.('Enter a whole creature quantity from 1 to 20.');
