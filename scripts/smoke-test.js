...
check('Campaign party membership commits before optional full-sheet sync', firebaseAuthJs.includes('upsertSharedCampaignCharacter') && firebaseAuthJs.includes('optional full-sheet snapshot') && firebaseAuthJs.includes('transaction.update(campaignRef'));
...
check('GM XP awards publish progression immediately', appJs.includes('saveCampaignCharacterProgress') && firebaseAuthJs.includes('saveCampaignCharacterProgress: async function') && firebaseAuthJs.includes("doc(db, 'campaigns', campaignId, 'characters', characterId)"));
...
check('XP rewards use an authoritative campaign progression stream', appJs.includes('publishCharacterProgression') && appJs.includes('saveCampaignProgression') && firebaseAuthJs.includes('saveCampaignProgression: async function') && firebaseAuthJs.includes("'systems', 'progression'"));
check('Player subscribes to the authoritative progression stream', firebaseAuthJs.includes('subscribeCampaignProgression: function') && dataSyncJs.includes('mergeRealtimeProgression') && dataSyncJs.includes('subscribeCampaignProgression'));
check('Late character autosaves cannot overwrite authoritative XP', dataSyncJs.includes('authoritativeProgression') && dataSyncJs.includes("options.source==='progression'") && dataSyncJs.includes('progressionSync:authority.progressionSync'));
check('XP sync scripts use cache-busted URLs', ['js/app.js?v=20260730-xp-sync-2','js/firebase-auth.js?v=20260730-xp-sync-2','js/data-sync.js?v=20260730-xp-sync-2'].every(src=>html.includes(src)));
