...
export function talentRules(talent,rank=1) {
  ...
  const zeal=['paladin:paladin-s-zeal','paladin:zeal'].includes(talent.id);
  if(zeal) for(let level=1;level<=rank;level++) for(const match of (talent.ranks[level] || '').matchAll(/^#### ([^\n(]+)\((\d+) Zeal Points\)\s*\n([\s\S]*?)(?=^#### |^### |$(?![\s\S]))/gm)) {
    choices.push({id:talentKey(match[1]),label:`${match[1].trim()} · ${match[2]} Zeal`,costs:{zp:Number(match[2])},description:match[3].trim()});
  }
  ...
  return {body,effects,defined:rankDefined(talent,rank),passive,costs,restore,bpGain,choices,cooldown,duration,
    activation:zeal || !passive || bpGain>0 || Object.values({...costs,...restore}).some(n=>n>0),needsSpell:talent.id==='spellblade:spell-weaving',
    blocked:mechanicsError || (!rankDefined(talent,rank)?'This rank has not been written yet.':variable || /GM-defined|Depends on/i.test(cooldown) || talent.id==='paladin:sacred-covenant'?'The source requires a variable or GM-defined cost. Resolve it with your GM.':''),
...
export function useLearnedTalent(character,talent,selection={},clock={}) {
  ...
  const costs={...(choice?.costs || rules.costs)}, restored={...rules.restore,...choice?.restore},bpGain=choice?.bpGain ?? rules.bpGain;
  if(rules.needsSpell) {
  ...
  if((rules.reset==='combat' || ['paladin:paladin-s-zeal','paladin:zeal'].includes(talent.id)) && !combatId) throw new Error('Start an encounter before using this talent.');
  ...
  if(choice?.id==='holy-restoration') restored.hp=2*Number(character.level || 0)+Math.min(5,Math.max(0,Math.floor(effectiveCharacteristicValue(character,'charisma',clock)/20)));
  ...
  const effect=sustained?{id:talent.id,talentId:talent.id,name:talent.name,rank,description:choice?.description || rules.effects,ac:choice?.id==='divine-guard'?2:rules.selfAC,encounterId:combatId,untilRound:choice?.id==='divine-guard'&&encounter?Number(encounter.round || 1)+1:encounter && durationMs?Number(encounter.round || 1)+Math.ceil(durationMs/6000):0,expiresAt:!encounter && durationMs?now+durationMs:0,ended:false,expired:false}:null;
